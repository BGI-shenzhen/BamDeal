...
cout<<""
"\n"
"\tUsage: DeteSV(beta) -l  <bam.list>  -o  <outPrefix>\n"
"\tUsage: DeteSV(beta) -I < A.bam B.bam > -o  <outPrefix>\n"
"\n"
"\t\t-i    <str>    input SAM/BAM files, delimited by space\n"
"\t\t-l    <str>    input list of SAM/BAM files\n"
"\t\t-o    <str>    prefix of output file\n"
"\n"
"\t\t-s    <int>    set the insert value,default[auto]\n"
"\t\t-d    <int>    set the insert standard deviation value,default[auto]\n"
"\t\t-r    <str>    input reference FASTA to get gap information\n"
"\t\t-m    <int>    set the minimum count of paired-end read in one cluster,default[6]\n"
"\n"
...
void More_bamSV_help()
{
cout<<""
"\n"
"\t\t1. DeteSV  -i <A.bam B.bam> -r  <Ref.fa> -o AAA  -m M\n"
"\t\t   DeteSV  -l  <bam.list> -r  <Ref.fa> -o AAA  -m M\n"
...
int bamSV_help01(int argc, char **argv , In3str1v * paraBamS )
{
	...
	for(int i = 1; i < argc ; i++)
	{
		...
		else if (flag  ==  "Insert"  ||  flag == "s" )
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			paraBamS->InInt=atoi(argv[i]);
		}
		else if (flag  ==  "SD" ||  flag == "d")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			paraBamS->InInt2=atoi(argv[i]);
		}
		else if (flag  ==  "MinDepth" ||  flag == "m")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
