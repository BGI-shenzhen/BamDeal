...
void bamCov_help()
{
	cout<<""
		...
		"\n"
		"\t\t-i    <str>     input SAM/BAM/cram files, delimited by space\n"
...
int bamCov_help01(int argc, char **argv , In3str1v * paraFA04 ,  In3str1v * SedparaFA04 )
{
...
//int main(int argc, char *argv[])
{
	...
	In3str1v *SedparaFA04 = new In3str1v;
	paraFA04->InInt=-1;
	paraFA04->InF=10000;
	if ((bamCov_help01(argc, argv, paraFA04,SedparaFA04)==0))
	{
		delete paraFA04 ;
		return 0 ;
	}
	...
	if(BamIn->format.format == htsExactFormat::cram)
	{
		...
		const char* ref_file = (paraFA04->InStr3).c_str();
	...
	OUTSTAT<<"#ID\tLength\tCoveredBase\tTotalBases\tCoverage%\tMeanDepth"<<endl;
	...
	if  ( (SedparaFA04->InInt)!=0)
	{

		string StatBed=PrefixO+".gene.stat";
		ofstream  OUTPP (StatBed.c_str());

		igzstream LIST ((SedparaFA04->InStr1).c_str(),ifstream::in); // igzstream
		if (!LIST.good())
		{
			cerr << "open bed File error: "<<(paraFA04->InStr1)<<endl;
			return  1;
		...
		OUTPP<<"#GeneID\tLength\tCoveredSite\tTotalDepth\tCoverage%\tMeanDepth\n";
		...
		for (auto iter = GeneLength.begin(); iter != GeneLength.end(); ++iter)
		{
			...
			double Coverage=NumCover*100.0/Length ;
			double MeanDepth=SumDepth*1.0/Length;
			OUTPP<<GeneID<<"\t"<<Length<<"\t"<<NumCover<<"\t"<<SumDepth<<"\t"<<setiosflags(ios::fixed)<<setiosflags(ios::right)<<setprecision(2)<<Coverage<<"\t"<<MeanDepth<<endl;
		...
		OUTPP<<"##RegionLength:\t"<<SS_Len<<"\tCoveredSite:\t"<<SS_Cov<<"\tCoverage%:\t"<<setiosflags(ios::fixed)<<setiosflags(ios::right)<<setprecision(2)<<Coverage<<"\tMeanDepth\t"<<MeanDepth<<endl;
