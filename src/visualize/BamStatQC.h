...
{
	if (argc <=1 ) {BamStatQC_help();return 0;}
	int file_count=0;
...
//int main(int argc, char *argv[])
{
	...
	ubit64_t  **cycle_qual = new ubit64_t *[ReadMaxL]; //¿ª±ڐΠ
	for(int i = 0; i < ReadMaxL; i++)
	{
		cycle_qual[i] = new ubit64_t [MAX_Q]; //¿ª±ځΠ
	...
	ubit64_t  **cycle_base = new ubit64_t *[ReadMaxL]; //¿ª±?
	for(int i = 0; i < ReadMaxL; i++)
	{
		cycle_base[i] = new ubit64_t [5]; //¿ª±ځΠ
