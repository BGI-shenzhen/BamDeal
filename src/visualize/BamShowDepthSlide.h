...
void bamDepSliding_help()
{
cout<<""
"\n"
"\tUsage: DepthSlide  -l  <bam.list>  -r  <Ref.fa> -o  <outPrefix>\n"
//"\tUsage: DepthSlide  -i <A.bam B.bam> -r <Ref.fa> -o <outprefix>\n"
"\tUsage: DepthSlide  -f  <DepthGC.wig.gz>   -o  <outPrefix>  -c chr1,chr2\n"
"\n"
...
void More_bamDepSliding_help()
{
cout<<""
"\n"
"\t\t1. DepthSlide -i <A.bam B.bam> -r <Ref.fa> -o AAA -k -q Q\n"
"\t\tDepthSlide -l <bam.list> -r <Ref.fa> -o AAA -k -q Q\n"
...
"\n"
"\t\t3. DepthSlide -i <A.bam B.bam> -r <Ref.fa> -o AAA -c Chr1,Chr3 -s 0.2\n"
...
{
	if (argc <=1 ) {bamDepSliding_help();return 0;}
	int file_count=0;
	for(int i = 1; i < argc ; i++)
	{
		...
		else if (flag  ==  "MaxY"  ||  flag  == "y")
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			MaxY=argv[i];
		}
		else if (flag  ==  "DrawChr" ||  flag  == "c" )
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
			OnlyChr=argv[i];
		...
		else if (flag  ==  "SlideRatio"  ||  flag  == "s" )
		{
			if(i + 1 == argc) {LogLackArg(flag); return 0;}
			i++;
		...
		{
			More_bamDepSliding_help();return 0;
		}
		else
		{
